...
// Gallery links were carried over from the original repository, not inferred
// from project names. These entries are not evidence of paid client work,
// currently operational services, or measured business outcomes.
const projects: { title: string; category: Exclude<Category, 'All'>; image: string; url: string; description: string }[] = [
  { title: 'Lueur Skin', category: 'E-Commerce', image: '/images/portfolio/lueur-skin.png', url: 'https://lueurskin.vercel.app/', description: 'Skincare storefront and product-browsing experience.' },
  { title: 'EMLL', category: 'Hospitality', image: '/images/portfolio/emll.png', url: 'https://emll.vercel.app/', description: 'Travel-experiences website focused on Marrakech.' },
  { title: 'RiadConnect', category: 'SaaS', image: '/images/portfolio/riadconnect.png', url: 'https://riadconnect.vercel.app/', description: 'A hospitality platform concept and booking interface.' },
  ...
  { title: 'EDU Platform', category: 'SaaS', image: '/images/portfolio/edu-platform.png', url: 'https://edumaroc.vercel.app', description: 'A learning-platform interface.' },
  { title: 'VitaCore', category: 'E-Commerce', image: '/images/portfolio/vitacore.png', url: 'https://vitapara.vercel.app/fr', description: 'Health-and-wellness storefront concept.' },
  { title: 'Sitey & AndK', category: 'SaaS', image: '/images/portfolio/sitey-andk.png', url: 'https://siteyandk.vercel.app', description: 'A marketplace concept for showcasing websites.' },
  { title: 'Transo', category: 'Web App', image: '/images/portfolio/transo.png', url: 'https://transomaroc.vercel.app', description: 'Sender and courier coordination interface.' },
  ...
  { title: 'Riad Dar Kader', category: 'Hospitality', image: '/images/portfolio/riad-dar-kader.png', url: 'https://mbndemo.vercel.app/fr', description: 'A riad website and booking interface.' },
];
    ...
    <PublicLayout>
      <section className="relative overflow-hidden px-4 sm:px-6 pt-36 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto relative z-10 text-center">
          ...
          <p className="text-slate-400 text-lg max-w-2xl mx-auto mt-6 leading-relaxed">A selection of product concepts, platforms and web experiences. These examples do not imply verified client engagements or commercial results. Ask about available references and project details.</p>
        </motion.div>
      </section>
      <section className="px-4 sm:px-6 pb-28">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-wrap items-center justify-center gap-2 mb-10" role="group" aria-label="Filter projects by type">
            {categories.map((item) => <button key={item} type="button" aria-pressed={category === item} onClick={() => setCategory(item)} className={`rounded-full text-sm px-5 py-2.5 border transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-violet-400 ${category === item ? 'bg-violet-600 border-violet-500 text-white' : 'bg-white/[0.03] border-white/10 text-slate-400 hover:text-white hover:border-white/25'}`}>{item}</button>)}
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((project, index) => <motion.article key={project.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: Math.min(index, 5) * 0.04 }} className="premium-surface rounded-2xl overflow-hidden group flex flex-col">
