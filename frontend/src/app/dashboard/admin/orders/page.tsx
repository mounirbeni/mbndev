    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="text-slate-400 text-sm">
          {orders.length} total · {formatCurrency(revenue)} revenue
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Pending Payment', value: pending,                              icon: Clock,       color: 'text-amber-400' },
          { label: 'Paid Orders',     value: orders.filter(o=>o.status==='paid').length, icon: CheckCircle2, color: 'text-green-400' },
          { label: 'Revenue',         value: formatCurrency(revenue),              icon: DollarSign,  color: 'text-primary-400' },
        ].map((s) => {
          const Icon = s.icon;
          return (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-white/5">
                    {['Client', 'Service', 'Title', 'Amount', 'Delivery', 'Status', 'Date', ''].map((h) => (
                      ...
                      >
                        ...
                        <td className="p-4">
                          {order.project ? (
                            ...
                            <span className="text-xs text-slate-600">No project</span>
                          )}
                        </td>
                      </motion.tr>
                    );
