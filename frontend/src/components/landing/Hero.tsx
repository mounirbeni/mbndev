...
import { ArrowRight, Zap, Shield, Code2, Headphones, MapPin, TrendingUp, CheckCircle2, Circle, BarChart2 } from 'lucide-react';
...
const stats = [
  { icon: Zap,        label: 'Fast Delivery',          desc: 'We respect deadlines and deliver on time, every time.' },
  { icon: Shield,     label: 'Modern & Secure',         desc: 'We use the latest technologies to ensure performance and security.' },
  { icon: Code2,      label: 'Clean Code',              desc: 'Scalable, maintainable, and built with best practices.' },
  { icon: Headphones, label: 'Support & Maintenance',   desc: "We're here for you even after delivery." },
];
...
export default function Hero() {
  return (
    <section
      id="home"
      className="relative min-h-screen flex flex-col justify-center pt-20 overflow-hidden bg-hero-gradient"
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[calc(100vh-5rem)]">
          ...
          <div className="flex flex-col justify-center">
            ...
            <motion.p {...fadeUp(0.35)} className="text-lg text-slate-400 mb-8 max-w-lg leading-relaxed">
            ...
            <motion.div {...fadeUp(0.55)}>
              ...
              <div className="flex flex-wrap gap-6 items-center">
                {clients.map((c) => (
                  <span key={c} className="text-slate-600 text-sm font-semibold uppercase tracking-wider hover:text-slate-400 transition-colors">
                    {c}
                  </span>
                ))}
              </div>
          ...
          {/* Right — Project Overview Dashboard Card */}
          <motion.div
          ...
          >
            <div className="animate-float w-full max-w-md">
              ...
              {/* Floating mini card — recent activity */}
              <motion.div
                animate={{ y: [0, -6, 0] }}
                transition={{ duration: 4, repeat: Infinity, delay: 0.5 }}
                className="absolute -bottom-6 -right-6 glass rounded-xl p-3 border border-white/10 w-44"
              >
